                ...
                @foreach($messages as $message)
                    <a href="message-chat?chat={{base64_encode(json_encode($message))}}">
                            <div class="row">
                                ...
                                <div class="col-md-2">
                                    <p style="color: grey">{{\Carbon\Carbon::parse($message->created_at)->format('g:i A')}}</p>
                                </div>
                                <div class="col-md-2">
                                    <p style="color: grey">{{\Carbon\Carbon::parse($message->created_at)->format('d/m/Y')}}</p>
                                </div>
                                <div class="col-md-10 offset-1"><p> {{$message->message}} </p></div>
                            </div>
                    </a>
                @endforeach
            @else
