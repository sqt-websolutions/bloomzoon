...
@endsection
        ...
        <div class="col-md-10">
            ...
            @if(count($messages = $user_metas->Message))
                ...
                @foreach($messages as $message)
                    <div class="card m-0 p-0" style="box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2); transition: 0.3s;">
                        <div class="row p-5 ml-5 mr-5" style="border-bottom: 1px solid #ddd;">
                            <a href="message-chat?chat={{base64_encode(json_encode($message))}}">
                                <div class="col-md-2 text-left"><span class="badge badge-danger">Admin</span></div>
                                <div class="col-md-4 text-left" style="color: #999; font-size: 18px;">{{ \Illuminate\Support\Carbon::parse($message->created_at)->format('g:i A')}}</div>
                                <div class="col-md-6 text-right" style="color: #999; font-size: 18px;">{{\Illuminate\Support\Carbon::parse($message->created_at)->format('d/m/y')}}</div>
                                <div class="text col-md-12 text-justify" style="color: #333; font-size: 18px;">{{$message->message}}</div>
                            </a>
                        </div>
                    </div>
                @endforeach
